...
function wpp_payments_form()  
{
	...
	if( is_array( $productIds ) ){
		foreach($productIds as $key=>$product){
			if(!wp_get_single_post($product) || !count(get_post_meta($product, 'payments', true)) ){
...
function wpp_payment_gateway_disable_country( $available_gateways ) {
	global $woocommerce;
		...
		foreach($items as $item){
			...
			if( count($itemsPays) ){
				foreach( $arrayKeys as $key )
					if( is_array( $itemsPays ) && !in_array( $key, $itemsPays ) )
...
add_filter( 'woocommerce_available_payment_gateways', 'wpp_payment_gateway_disable_country' );
?>